...
import { type InjectedExtension, connectInjectedExtension } from "polkadot-api/pjs-signer";
...
export const ExtensionProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  const selectedExtensions = useSyncExternalStore(extensionsStore.subscribe, extensionsStore.getSnapshot);

  if (availableExtensions.length === 0) return <div>No extension provider detected</div>;
