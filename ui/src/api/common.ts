...
import { AccountId, type PolkadotSigner, type TypedApi } from "polkadot-api";
import { Binary, Enum, type SS58String } from "polkadot-api";
...
export const getNativeAsset = (parents: number, amount: bigint): XcmVersionedAssets =>
...
export const fromAssetHubToForeign = (
  ...
  dest: XcmVersionedLocation.V4({
    parents: 2,
    interior: XcmV3Junctions.X2([XcmV3Junction.GlobalConsensus(network), XcmV3Junction.Parachain(parachainId)]),
  }),
  beneficiary: getBeneficiary(to ?? from.publicKey),
  assets,
  fee_asset_item: 0,
...
export const watchAccoutFreeBalance = (api: {
  ...
  return (account: SS58String) =>
    combineLatest([from(edPromise), api.query.System.Account.watchValue(account, "best")]).pipe(
      map(([ed, { data }]) => {
        const spendableBalance = data.free - (data.frozen - data.reserved > ed ? data.frozen - data.reserved : ed);
...
export const watchForeingAssetAccoutFreeBalance =
  ...
  (account: SS58String) =>
    api.query.ForeignAssets.Account.watchValue(asset, account, "best").pipe(
      map((data) => (data?.status && Enum.is(data.status, "Liquid") ? data.balance : 0n)),
    );
