...
const ksm: AssetInChain = {
  ...
  teleport: {
    ksm: (...args) => api.tx.PolkadotXcm.transfer_assets(fromAssetHubToRelay(...args)),
    dotAh: (from, amount, to) =>
      api.tx.PolkadotXcm.transfer_assets(
        fromAssetHubToForeign(XcmV3JunctionNetworkId.Polkadot(), 1000, getNativeAsset(1, amount), from, to),
      ),
  },
};
...
const dotInKsmAh: Parameters<typeof XcmVersionedLocation.V4>[0] = {
  parents: 2,
  interior: XcmV3Junctions.X1(XcmV3Junction.GlobalConsensus(XcmV3JunctionNetworkId.Polkadot())),
};
